          ...
          <el-row :gutter="24">
            ...
            <el-col :span="18">
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item label="本次预收" prop="bankAccountId">
...
const save = async () => {
  proxy.$refs["salesOrderForm"].validate(valid => {
    if (valid) {
      doSave()
    }
  })
...
const doSave = async (orderStatus = 0) => {
  //验证salesOrderForm表单
  salesOrderForm.value?.validate((valid) => {
    // 校验
    if (!valid) {
...
const doFinishEdit = async () => {
  salesOrderForm.value?.validate(async (valid) => {
    // 校验
    if (!valid) {
      return ElMessage.error('请填写必填项')
    }
