  ...
  <div class="app-container">
    ...
    <el-card class="mt20">
      ...
      <el-row :gutter="10" class="mb8" type="flex" justify="space-between">
        <el-col :span="6"><span style="font-size: large">销售订单</span></el-col>
        <el-col :span="1.5">
          <el-button
            type="primary"
            plain
...
function handleQuery() {
  ...
  /** 新增按钮操作 */
  function handleAdd() {
    proxy.$router.push({ path: "/sales/orderEdit" });
}

/** 修改按钮操作 */
function handleUpdate(row) {
  proxy.$router.push({ path: "/sales/orderEdit",  query: { id: row.id } });
}
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除销售订单编号为"' + docNo + '"的数据项？').then(function() {
    loading.value = true;
...
function handleExport() {
  proxy.download('sales/order/export', {
    ...queryParams.value
